...
//   priority_queue<heap_node, vector<heap_node>,compare> Heap;
    ...
    void Clear()
    {
        for(uint32_t i = 0; i< K; i++){
            heap[i].key = 0;
            heap[i].value = 0;
            for( uint32_t j = 0;j < lu_d;j++) heap[i].index[j] = 0;
        }
...
public:
    LUSketch(int total_memory, int top_k)
    {
        K = top_k;
    ...
    {
        cout<<"LUSketch"<<endl<<"heap memory: "<<heap_memory<<"B"<<endl;
        cm_print_basic_info();
    }
        ...
        if(flag == 0){
            ...
            if(heap[0].value < value){
               ...
               for( int t = floor(K/2); t>=0;t--){
                    heap_count ++;//record the number of minheapify operations.
                    MINHEAPIFY(t);
               }
